...
const Edge = require('./directed-edge')
function EdgeWeightedDigraph(V = 0, E = 0, data = []) {
    this.data = data
    this.V = V
    this.E = E
    this.adj = []
...
EdgeWeightedDigraph.prototype = {
    init() {
        ...
        for (let i = 0; i < this.E; i++) {
            const v = parseFloat(this.data[start++])
            const w = parseFloat(this.data[start++])
            const weight = parseFloat(this.data[start++])
